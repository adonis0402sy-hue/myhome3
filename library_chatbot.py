...
# 페이지 설정: 넓은 레이아웃과 별자리 아이콘으로 천문학적 분위기 연출
st.set_page_config(
    layout="wide", 
    page_title="별자리 챗봇", 
...
def initialize_components(selected_model):
    ...
    try:
        ...
        st.error(f"❌ PDF 파일 '{file_path}' 로드 실패. 파일이 경로에 있는지 확인해주세요: {str(e)}")
        st.stop()
    ...
    # 질문-답변 시스템 프롬프트 (별자리 페르소나 강화)
    qa_system_prompt = """You are a highly knowledgeable astrologer and astronomer assistant for question-answering tasks. \
    Use the following pieces of retrieved context to answer the question. \
    If you don't know the answer, just say that you don't know. \
    Keep the answer perfect. please use imogi with the answer.
    대답은 한국어로 하고, 존댓말을 써줘. 당신은 별자리, 신화, 천문학적 사실에 대해 전문적인 지식을 가지고 있습니다.
...
try:
    with st.spinner("🔧 우주의 지식을 로드하는 중... 잠시만 기다려주세요"):
        rag_chain = initialize_components(option)
    st.success("✅ 별자리 챗봇이 준비되었습니다! 별들의 이야기에 귀 기울여보세요. 🌠")
except Exception as e:
...
if "messages" not in st.session_state:
    st.session_state["messages"] = [{"role": "assistant", 
                                     "content": "궁금한 별자리의 이름이나 신화에 대해 물어보시면 제가 알고 있는 모든 정보를 알려드리겠습니다! 🌟"}]

for msg in chat_history.messages:
    st.chat_message(msg.type).write(msg.content)


if prompt_message := st.chat_input("별자리에 대해 질문해보세요."):
    st.chat_message("human").write(prompt_message)
    with st.chat_message("ai"):
        with st.spinner("별들이 속삭이는 지식을 찾고 있습니다..."):
            ...
            st.write(answer)
            with st.expander("✨ 참고 문서 확인 (별자리의 근거)"):
